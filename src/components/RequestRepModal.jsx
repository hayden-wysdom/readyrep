...
export default function RequestRepModal({ onClose }) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('send-rep-request', {
        body: {
    ...
    <div className="request-rep-overlay" onClick={onClose}>
      <div className="request-rep-modal" onClick={(e) => e.stopPropagation()}>
        <div className="request-rep-header">
          <h2>Request a Representative</h2>
        ...
        {submitted ? (
          <div className="request-rep-success">
            ...
            <p>We'll work on connecting you with a representative. You'll receive an email from our team in the next 24-48 hours.</p>
          ...
          <form onSubmit={handleSubmit} className="request-rep-form">
            ...
            <div className="form-group" ref={companyDropdownRef}>
              <label htmlFor="reqCompany">Company</label>
            ...
            <div className="form-group">
              <label htmlFor="reqDevice">What device are you looking for?</label>
              <textarea
                id="reqDevice"
                value={form.deviceDescription}
                onChange={(e) => setForm({ ...form, deviceDescription: e.target.value })}
                placeholder="Describe the device or product you're interested in..."
                rows={4}
                required
              />
            </div>
